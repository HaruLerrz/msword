...
#ifdef OPUS_X64
struct OPUS_FONT_ENUM_CONTEXT
	{
	struct STTB ***phsttbPaf;
	struct PAF *ppaf;
	};
int CALLBACK FontNameEnum(const LOGFONTA *, const TEXTMETRICA *, DWORD, LPARAM);
int CALLBACK FontSizeEnum(const LOGFONTA *, const TEXTMETRICA *, DWORD, LPARAM);
#endif
...
struct STTB ***phsttbPaf;
{
	...
	int tc;
#ifdef OPUS_X64
	struct OPUS_FONT_ENUM_CONTEXT context;
#endif
...
#ifdef OPUS_X64
	context.phsttbPaf = phsttbPaf;
	context.ppaf = 0;
	EnumFontsA(vpri.hdc, NULL, FontNameEnum, (LPARAM)&context);
#else
...
#ifdef OPUS_X64
int CALLBACK FontNameEnum(const LOGFONTA *lplf, const TEXTMETRICA *lptm,
		DWORD fty, LPARAM lParam)
#else
...
#ifdef OPUS_X64
	struct OPUS_FONT_ENUM_CONTEXT *pdw =
			(struct OPUS_FONT_ENUM_CONTEXT *)lParam;
	struct STTB **hsttbPaf = *pdw->phsttbPaf;
#else
...
#ifdef OPUS_X64
	pdw->ppaf = ppaf;
	EnumFontsA(vpri.hdc, (LPCSTR)pffn->szFfn, FontSizeEnum, (LPARAM)pdw);
#else
	dw.ppaf = ppaf;
...
#ifdef OPUS_X64
int CALLBACK FontSizeEnum(const LOGFONTA *lplf, const TEXTMETRICA *lptm,
		DWORD fty, LPARAM lParam)
#else
...
#endif
#ifdef OPUS_X64
	AddHpsToPaf(((struct OPUS_FONT_ENUM_CONTEXT *)lParam)->ppaf,
			UMultDiv(lptm->tmHeight - lptm->tmInternalLeading,
			144 /* hpsInch */, vfli.dyuInch));
	return fTrue;
#else
...
#ifdef OPUS_X64
csconst CHAR * const rgstSection [] =
{
	"intl",
	"Microsoft Word",
};
...
{
	BYTE istSection;
	const CHAR *stProfile;
	const CHAR *stDefault;
}
rgpst [] =
{
	{ iszIntl, "sDecimal", "." },
	...
	{ iszIntl, "iCurrency", "0" },
	{ iszIntl, "iDigits", "2" },
	{ iszApp, "DOC-Extension", ".DOC" },
	{ iszApp, "DOT-Extension", ".DOT" },
	{ iszApp, "BAK-Extension", ".BAK" },
	{ iszApp, "", "" },
	{ iszApp, "LoadCore", "" },
	{ iszApp, "Conversion", "" },
	{ iszApp, "DrawAppName", "EDITPIC.EXE" },
	{ iszApp, "DrawAppClass", "EDITPIC" },
	{ iszIntl, "iCountry", "001" },
	{ iszIntl, "sP", "P" },
	{ iszIntl, "sS", "S" },
	{ iszIntl, "iTime", "0" },
	{ iszApp, "NovellNet", "" },
...
#ifdef OPUS_X64
	CchCopySz(rgstSection[rgpst[ipst].istSection], szSection);
	CchCopySz(rgpst[ipst].stProfile, szProfile);
	CchCopySz(rgpst[ipst].stDefault, szDefault);
#else
...
int UlmFromDrvWinIni()
{
	...
	if (ulm == ulmNil)
		...
		if (vwWinVersion > 0x0203)
			...
			if ((hDriver = LoadLibrary((LPSTR)szDriver)) == NULL)
				/* Cannot load library */
				return ulm;
