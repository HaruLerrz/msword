...
#ifdef OPUS_X64
CHAR            stNormalDot[] = { 6, 'N', 'O', 'R', 'M', 'A', 'L' };
#else
...
/* create application top-level window */
#ifdef OPUS_X64
	/* Registering a top-level window with an invalid submenu reports only
	 * ERROR_INVALID_MENU_HANDLE.  Validate the migrated menu tree here so the
	 * failing pointer-width boundary is named precisely. */
	{
	int iMenuItem;
	int cMenuItem;
	if (!IsMenu(vhMenu) || (cMenuItem = GetMenuItemCount(vhMenu)) < 0)
		{
		ReportSz("Selected app menu handle is invalid");
		goto InzFailed;
		}
	for (iMenuItem = 0; iMenuItem < cMenuItem; ++iMenuItem)
		{
		UINT menuState = GetMenuState(vhMenu, iMenuItem, MF_BYPOSITION);
		if (menuState == (UINT)-1)
			{
			ReportSz("Cannot read selected app menu item");
			goto InzFailed;
			}
		if ((menuState & MF_POPUP) != 0 &&
				!IsMenu(GetSubMenu(vhMenu, iMenuItem)))
			{
			ReportSz("Selected app submenu handle is invalid");
			goto InzFailed;
			}
		}
	}
	...
	if (CreateWindow((LPSTR)szClsApp,
		...
		{
#ifdef OPUS_X64
		OpusX64ReportWin32("App Window creation failure", GetLastError(),
				__FILE__, __LINE__);
#else
...
#ifdef OPUS_X64
csconst char stCbtGlobalDot[] = { 7, 'C', 'B', 'T', 'N', 'O', 'R', 'M' };
#else
...
#ifdef OPUS_X64
LRESULT CALLBACK StartWndProc(HWND hwnd, UINT wm, WPARAM wParam, LPARAM lParam)
#else
...
FCreateSysMenu()
{
...
#ifdef OPUS_X64
	/* OBM_OLD_CLOSE was a USER-provided Win2 bitmap strip and is no longer
	 * exposed by Win64.  Recreate the same child-window control bitmap at the
	 * current menu metrics; the surrounding Opus menu insertion code remains
	 * unchanged. */
	HDC hdcScreen;
	HDC hdcBitmap;
	HBITMAP hbmpOld;
	...
	hbmpSystem = CreateCompatibleBitmap(hdcScreen, dxMenu, dyMenu);
	ReleaseDC(NULL, hdcScreen);
	if (hbmpSystem == NULL)
		{
		DeleteDC(hdcBitmap);
